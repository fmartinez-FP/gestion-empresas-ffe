        ...
        <nav class="bg-white dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700 sticky top-0 z-50 shadow-sm">
            <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                <div class="flex h-16 items-center justify-between gap-4">
                    ...
                    <!-- Navegación central — barra primaria -->
                    @auth
                    <nav class="hidden md:flex items-center gap-1">
                        <a href="{{ route('empresas.index') }}" class="nav-link px-3 py-2 rounded-lg text-sm font-medium flex items-center gap-2 {{ request()->routeIs('empresas.*') || request()->routeIs('seguimientos.*') || request()->routeIs('colocaciones.*') ? 'bg-primary-50 text-primary-700 dark:bg-primary-900/30 dark:text-primary-400' : 'text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700' }}">
                        ...
                        @endcan
                        @admin
                        <a href="{{ route('admin.usuarios.index') }}" class="nav-link px-3 py-2 rounded-lg text-sm font-medium flex items-center gap-2 {{ request()->routeIs('admin.*') && !request()->routeIs('admin.ciclos.*') && !request()->routeIs('admin.curriculum.*') && !request()->routeIs('admin.calendario-ffe.*') ? 'bg-primary-50 text-primary-700 dark:bg-primary-900/30 dark:text-primary-400' : 'text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700' }}">
            ...
            <div id="mobile-menu" class="hidden md:hidden border-t border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800">
                <div class="px-4 py-3 space-y-1">
                    ...
                    @endcan
                    @admin
                    <a href="{{ route('admin.usuarios.index') }}" class="block px-3 py-2 rounded-lg text-sm font-medium {{ request()->routeIs('admin.*') && !request()->routeIs('admin.ciclos.*') && !request()->routeIs('admin.curriculum.*') && !request()->routeIs('admin.calendario-ffe.*') ? 'bg-primary-50 text-primary-700' : 'text-slate-600' }}">Administración</a>
                    @endadmin
                    {{-- Subnav móvil Empresas --}}
                    @if(request()->routeIs('empresas.*') || request()->routeIs('seguimientos.*') || request()->routeIs('colocaciones.*'))
                    <div class="border-t border-slate-100 mt-1 pt-1">
                        <a href="{{ route('empresas.index') }}" class="block px-3 py-1.5 rounded-lg text-xs font-medium {{ request()->routeIs('empresas.*') ? 'text-primary-700' : 'text-slate-500' }}">→ Empresas</a>
                        <a href="{{ route('seguimientos.index') }}" class="block px-3 py-1.5 rounded-lg text-xs font-medium {{ request()->routeIs('seguimientos.*') ? 'text-primary-700' : 'text-slate-500' }}">→ Agenda</a>
                        <a href="{{ route('colocaciones.index') }}" class="block px-3 py-1.5 rounded-lg text-xs font-medium {{ request()->routeIs('colocaciones.*') ? 'text-primary-700' : 'text-slate-500' }}">→ Histórico</a>
                    </div>
                    @endif
                    {{-- Subnav móvil Alumnos --}}
                    @if(request()->routeIs('alumnos.*') || request()->routeIs('asignaciones.*'))
                    <div class="border-t border-slate-100 mt-1 pt-1">
                        <a href="{{ route('alumnos.index') }}" class="block px-3 py-1.5 rounded-lg text-xs font-medium {{ request()->routeIs('alumnos.*') && !request()->routeIs('asignaciones.*') ? 'text-primary-700' : 'text-slate-500' }}">→ Alumnos</a>
                        <a href="{{ route('alumnos.index') }}?vista=asignaciones" class="block px-3 py-1.5 rounded-lg text-xs font-medium {{ request()->routeIs('asignaciones.*') ? 'text-primary-700' : 'text-slate-500' }}">→ Asignaciones</a>
                        @if(Route::has('asignaciones.seguimientos.index'))
                        <a href="{{ route('alumnos.index') }}?vista=cuaderno" class="block px-3 py-1.5 rounded-lg text-xs font-medium {{ request()->routeIs('asignaciones.seguimientos.*') || request()->routeIs('asignaciones.calendario.*') ? 'text-primary-700' : 'text-slate-500' }}">→ Cuaderno de seguimiento</a>
                        @endif
                    </div>
                    @endif
                </div>

...
<div class="max-w-3xl mx-auto space-y-6">
    ...
    <div class="flex items-center gap-4">
        ...
        <div>
            <h1 class="text-2xl font-bold text-slate-800 dark:text-white">Registrar Contacto</h1>
            <p class="text-slate-500 dark:text-slate-400 mt-1">{{ $empresa->nombre }}</p>
        </div>
    </div>

    <!-- Formulario -->
    <form method="POST" action="{{ route('contactos.store', $empresa) }}" enctype="multipart/form-data" class="space-y-6">
        @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select name="tipo" id="tipo" required
                        ...
                        <option value="llamada" {{ old('tipo') == 'llamada' ? 'selected' : '' }}>📞 Llamada telefónica</option>
                        <option value="email" {{ old('tipo') == 'email' ? 'selected' : '' }}>📧 Email</option>
                        <option value="visita" {{ old('tipo') == 'visita' ? 'selected' : '' }}>📍 Visita presencial</option>
                        <option value="reunion_online" {{ old('tipo') == 'reunion_online' ? 'selected' : '' }}>💻 Reunión online</option>
                ...
                <div>
                    ...
                    <select name="resultado" id="resultado" required
                            class="w-full px-4 py-2.5 border border-slate-200 dark:border-slate-600 rounded-xl bg-white dark:bg-slate-700 text-slate-800 dark:text-white focus:outline-none focus:ring-2 focus:ring-primary-500">
                        <option value="exitoso" {{ old('resultado') == 'exitoso' ? 'selected' : '' }}>✅ Exitoso</option>
                        <option value="sin_respuesta" {{ old('resultado') == 'sin_respuesta' ? 'selected' : '' }}>❌ Sin respuesta</option>
                        <option value="pendiente" {{ old('resultado') == 'pendiente' ? 'selected' : '' }}>⏳ Pendiente de respuesta</option>
                        <option value="cita_programada" {{ old('resultado') == 'cita_programada' ? 'selected' : '' }}>📅 Cita programada</option>
                    </select>
                </div>
                ...
                <div>
                    ...
                    <input type="datetime-local" name="fecha_contacto" id="fecha_contacto" required
                           value="{{ old('fecha_contacto', now()->format('Y-m-d\TH:i')) }}"
                           class="w-full px-4 py-2.5 border border-slate-200 dark:border-slate-600 rounded-xl bg-white dark:bg-slate-700 text-slate-800 dark:text-white focus:outline-none focus:ring-2 focus:ring-primary-500">
                </div>
                ...
                <div>
                    ...
                    <input type="datetime-local" name="fecha_seguimiento" id="fecha_seguimiento"
                           ...
                           class="w-full px-4 py-2.5 border border-slate-200 dark:border-slate-600 rounded-xl bg-white dark:bg-slate-700 text-slate-800 dark:text-white focus:outline-none focus:ring-2 focus:ring-primary-500">
                    <p class="mt-1 text-xs text-slate-500 dark:text-slate-400">Opcional. Útil para programar recordatorios.</p>
                </div>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="notas" id="notas" rows="4"
                              ...
                              class="w-full px-4 py-2.5 border border-slate-200 dark:border-slate-600 rounded-xl bg-white dark:bg-slate-700 text-slate-800 dark:text-white focus:outline-none focus:ring-2 focus:ring-primary-500 resize-none">{{ old('notas') }}</textarea>
                </div>
            </div>
        </div>

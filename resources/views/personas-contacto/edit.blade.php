...
@section('title', 'Editar Persona de Contacto')
...
<div class="max-w-2xl mx-auto space-y-6">
    ...
    <div class="flex items-center gap-4">
        ...
        <div>
            <h1 class="text-2xl font-bold text-slate-800 dark:text-white">Editar Persona de Contacto</h1>
            <p class="text-slate-500 dark:text-slate-400 mt-1">{{ $empresa->nombre }}</p>
        </div>
    </div>

    <form method="POST" action="{{ route('personas-contacto.update', [$empresa, $persona]) }}" class="space-y-6">
        @csrf
        @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea id="notas" name="notas" rows="3"
                              class="w-full px-4 py-2.5 border border-slate-200 dark:border-slate-600 rounded-xl bg-white dark:bg-slate-700 text-slate-800 dark:text-white focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-transparent resize-none">{{ old('notas', $persona->notas) }}</textarea>
                </div>

                <div class="md:col-span-2">
                    <label class="flex items-center gap-3 cursor-pointer">
                        <input type="checkbox" name="principal" value="1"
                               {{ old('principal', $persona->principal) ? 'checked' : '' }}
                               class="w-5 h-5 rounded border-slate-300 text-primary-600 focus:ring-primary-500">
                        <span class="text-sm font-medium text-slate-700 dark:text-slate-300">Contacto principal</span>
                    </label>
                </div>

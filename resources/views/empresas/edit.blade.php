...
<div class="max-w-4xl mx-auto space-y-6">
    ...
    >
        ...
        @if(isset($empresa))
            @method('PUT')
        <input type="hidden" name="ciclos_enviados" value="1">
        @endif
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input 
                        ...
                        class="w-full px-4 py-2.5 border border-slate-200 dark:border-slate-600 rounded-xl bg-white dark:bg-slate-700 text-slate-800 dark:text-white focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-transparent"
                    >
                    <p class="mt-1 text-xs text-slate-500 dark:text-slate-400">El convenio tendrá una vigencia de 4 años desde esta fecha</p>
                </div>
        ...
        {{-- Reasignación de profesor (solo admin y responsables) --}}
        @if(isset($profesores) && $profesores && isset($empresa))
        <div class="bg-amber-50 dark:bg-amber-900/20 rounded-2xl shadow-sm border border-amber-200 dark:border-amber-800 p-6">
            ...
            <p class="text-sm text-amber-700 dark:text-amber-300 mb-4">
                Puedes reasignar esta empresa a otro profesor del sistema.
                @if(auth()->user()->esResponsableCiclo())
                    <span class="font-medium">Solo aplicable a empresas adscritas a tus ciclos formativos.</span>
                @endif
            </p>

            <div>
                <label for="creador_id" class="block text-sm font-medium text-amber-800 dark:text-amber-200 mb-2">
                    Profesor asignado
                </label>
                <select 
                    name="creador_id" 
                    id="creador_id" 
                    class="w-full px-4 py-2.5 border border-amber-300 dark:border-amber-600 rounded-xl bg-white dark:bg-slate-700 text-slate-800 dark:text-white focus:ring-2 focus:ring-amber-500"
                >
                    @foreach($profesores as $profesor)
                    <option value="{{ $profesor->id }}" {{ $empresa->creador_id == $profesor->id ? 'selected' : '' }}>
                        {{ $profesor->nombre }} ({{ $profesor->username }}) - {{ ucfirst(str_replace('_', ' ', $profesor->rol)) }}
                    </option>
                    @endforeach
                </select>
                <p class="mt-1.5 text-xs text-amber-600 dark:text-amber-400">
                    Al cambiar el profesor, este será el nuevo responsable de la empresa.
                </p>
            </div>
        </div>
        @endif

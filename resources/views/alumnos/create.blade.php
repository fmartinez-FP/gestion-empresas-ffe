        ...
        <div>
            ...
            @if($ciclos->isEmpty())
                <div class="rounded-lg border border-amber-300 bg-amber-50 px-3 py-2 text-sm text-amber-800">
                    No hay ciclos formativos registrados. Crea uno desde el panel de administración antes de añadir alumnos.
                </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
            ...
            <div>
                ...
                <select name="numero_curso" required
                        class="w-full rounded-lg border @error('numero_curso') border-red-400 @else border-gray-300 @enderror px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500">
                    <option value="1" {{ old('numero_curso') == '1' ? 'selected' : '' }}>1º curso</option>
                    <option value="2" {{ old('numero_curso', '2') == '2' ? 'selected' : '' }}>2º curso</option>
                </select>
                @error('numero_curso')<p class="text-xs text-red-600 mt-1">{{ $message }}</p>@enderror
            </div>
        </div>
